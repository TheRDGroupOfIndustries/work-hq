...
export default function RaiseTicket() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/ticket/create/${projectName}`, {
        method: 'POST',
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <div className="flex flex-row gap-2 justify-end">
          ...
          <button type="submit" className="flex flex-row items-center py-3 px-5 gap-2 shadow-[3px_3px_10px_0px_#789BD399,5px_5px_15px_0px_#00000099_inset,-3px_-3px_10px_0px_#FFFFFF] rounded-xl text-[#ffffff]  text-nowrap bg-primary-blue">
          <Plus color="#ffffff" />
          Raise Ticket
        </button>
      </div>
