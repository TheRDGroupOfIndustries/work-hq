...
export default function MeetingsDetails() {
  ...
  return (
    <MainContainer role={ROLE}>
      <Headline btnText="Create A Meeting" />
    ...
    <div className="flex flex-row items-center justify-between w-full hover:shadow-neuro-3 p-3 sm:p-4 md:p-5 lg:p-6">
      <div className="flex flex-row items-center gap-4">
        ...
        <div className="flex flex-col">
          <h3 className={`text-lg font-semibold ${details.status === "In Progress" || details.status === "Requested"
            ? "text-[#155EEF]"
            : "text-gray-800"
            }`}>
            {details.title}
      ...
      <div className="flex flex-col items-center justify-center gap-2">
        <p className={`text-base ${details.status === "Overdue" ? "text-[#FF3B30]" :
          details.status === "Completed" ? "text-[#34C759]" :
            "text-[#155EEF]"
          } font-normal`}>
          {details.status}
        </p>
        {(details.status === "In Pregress" || details.status === "Requested") && (
          <SquareButton
